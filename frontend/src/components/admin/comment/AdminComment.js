...
class AdminComment extends Component {
    ...
    editComment = data => {
        this.props.commentAction(data, UPDATE_COMMENT, MANAGE_COMMENT_URL)
    }

    deleteComment = data => {
       this.props.commentAction(data, DELETE_COMMENT, MANAGE_COMMENT_URL)
    }

    componentDidMount() {
        this.props.commentAction(null, GET_COMMENT, MANAGE_COMMENT_URL)
    }

    render() {
        const { classes } = this.props;
        const data = this.props.comment.comment;
        const { t } = this.props;
            ...
            <div>
                ...
                <Grid container justify="flex-end">
                     <Button
                      ...
                      href={ADD_COMMENT_URL}
                    >
                         {t("admin.manageComment.title")}
                    </Button>
                </Grid>
                         ...
                         options={{
                          ...
                          columns={
                              [
                                ...
                                { title: t("admin.manageCategory.content"), field: 'content' },
                                { title: t("admin.manageComment.post"), field: 'post.slug' },
                                { title: t("admin.manageCategory.content"), field: 'content' },
                                { title: t("admin.manageComment.createdBy"), field: 'created_by' },
                                { title: t("admin.manageComment.isDisplayed"), field: 'is_displayed' },
                          ]
                          }
                          data={data}
                          editable={{
                            ...
                            onRowDelete: oldData =>
                              new Promise((resolve, reject) => {
                                setTimeout(() => {
                                  ...
                                  this.deleteComment(dataDelete[index]);
...
export default connect(mapStateToProps, {addUserAction, commentAction})(withTranslation()(withStyles(styles) (withRouter(AdminComment))));
