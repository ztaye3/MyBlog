...
class AdminPost extends Component {
    ...
    editPost = data => {
        this.props.postAction(data, UPDATE_POST, MANAGE_POST_URL)
    }

    deletePost = data => {
       this.props.postAction(data, DELETE_POST, MANAGE_POST_URL)
    }

    componentDidMount() {
        this.props.postAction(null, GET_POST, MANAGE_POST_URL)
    }

    render() {
        const { classes } = this.props;
        const data = this.props.post.post;
        const { t } = this.props;
                ...
                <div style={{ maxWidth: "100%"}}>
                    ...
                    <MaterialTable
                         ...
                         options={{
                          ...
                          title={t("admin.managePost.manage")}
                          columns={
                              [

                                { title: 'Id', field: 'id', type: 'numeric', editable: 'never' },
                                { title: t("admin.managePost.title"), field: 'title' },
                                { title: t("admin.managePost.slug"), field: 'slug' },
                                { title: t("admin.managePost.excerpt"), field: 'excerpt' },
                                { title: t("admin.managePost.content"), field: 'content' },
                                { title: t("admin.managePost.status"), field: 'status' },
                                { title: t("admin.managePost.category"), field: 'category.name' },
                                { title: t("admin.managePost.createdBy"), field: 'created_by.email' },
                            ...
                            onRowDelete: oldData =>
                              new Promise((resolve, reject) => {
                                setTimeout(() => {
                                  ...
                                  this.deletePost(dataDelete[index]);
...
export default connect(mapStateToProps, {addUserAction, postAction})(withTranslation()(withStyles(styles) (withRouter(AdminPost))));
